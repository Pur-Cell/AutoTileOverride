...
namespace PurCell
{
    ...
    public class AutoTileOverride : TileBase
    {
        ...
        [Serializable]
        public class AutoTileOverrideData
        {
            [SerializeField]
        ...
        [Serializable]
        public class AutoTileDictionary : SerializedDictionary<uint, AutoTileOverrideData>
        {
        }
        ...
        public override void GetTileData(Vector3Int position, ITilemap itilemap, ref TileData tileData)
        {
            ...
            if (m_AutoTileDictionary.TryGetValue(num, out var value))
            {
                ...
                if (value.spriteList.Count > 0)
                {
                    if (m_Random)
                    {
                        long num4 = position.x;
                        num4 = num4 + 2882343476u + (num4 << 15);
                        num4 = (num4 + 159903659) ^ (num4 >> 11);
                        num4 ^= position.y;
                        num4 = num4 + 1185682173 + (num4 << 7);
                        num4 = (num4 + 3197579439u) ^ (num4 << 11);
                        Random.State state = Random.state;
                        Random.InitState((int)num4);
        ...
        public void AddSprite(Sprite sprite, Texture2D texture, uint mask)
        {
            ...
            {
                value = new AutoTileOverrideData();
        ...
        public void Validate()
        {
            ...
            foreach (KeyValuePair<uint, AutoTileOverrideData> item3 in m_AutoTileDictionary)
            {
                AutoTileOverrideData value = item3.Value;
                int num = 0;
